...
if (bd_verificar_privilegios('aporte_patronal2.php',$_SESSION['usuario']['nivel_id'])!='CONCEDER')
{
...
foreach ($nominas_iutet as $m=>$n)
{
	...
	for($i=6; $i<=$numRows; $i++)
	{
		...
		if ($exist < 1)
		{
			enviar_sql("INSERT INTO nomina (id,archivo_id,aporte_patronal,docente_id) 
					VALUES ('','$id_archivo','$aporte','$cedula');");
		}
		$z = $z + 1;
	}
}
$datos_total = sql2array("SELECT docentes_iutet.cedula, nomina.aporte_patronal FROM docentes_iutet INNER JOIN nomina ON nomina.docente_id = docentes_iutet.cedula
						WHERE nomina.aporte_patronal > 0");
$fecha_ini1 = f2f($fecha_ini);
$fecha_fin1 = f2f($fecha_fin);
$tipo = bd_obt_tipo_nomina($tipo);

foreach ($datos_total as $g=>$t)
{
	$lista[$g] = array ($datos_total[$g][cedula],$fecha_ini1,$fecha_fin1,0,$datos_total[$g]['aporte_patronal']);
}
$fp = fopen('./upload/archivos_figs/aporte_patronal'."$fecha_ini1"."$fecha_fin1"."$tipo".'.xls','w');
foreach ($lista as $campos) {
    fputcsv($fp, $campos);
}
fclose($fp);

$nombre_archivo = "aporte_patronal"."$fecha_ini1"."$fecha_fin1"."$tipo".".xls";
$fecha = date('Y-m-d');
