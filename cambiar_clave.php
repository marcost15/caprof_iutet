...
$f1->textField($star.'Login Anterior','login_viejo',FH_STRING,15,30);
$f1->textField($star.'Nuevo Login','login_nuevo',FH_STRING,15,30);
$f1->passField($star.'Clave actual','clave_actual',FH_STRING,15,30);
$f1->passField($star.'Nueva clave','clave_nueva',FH_PASSWORD,15,30);
$f1->passField($star.'Confirmar Clave','clave_conf',FH_PASSWORD,15,30);
...
{  
	...
	$resp = sql2value("SELECT COUNT(*) FROM usuarios WHERE login = '$login1'");
	if ($resp == 0)
	{
	...
	if ($mark == true)
	{
		...
		?>
				...
				window.alert('CAMBIO DE CLAVE REALIZADO CORRECTAMENTE');
